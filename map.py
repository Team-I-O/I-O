...
def mapping():
    ...
    for mark in rd.sample([2, 3, 4], 3):
        landmark_pos.append([rd.randint(0, 4), rd.randint(0, 4), mark])  # 위치 중복 없게 수정 필요
    ...
    def random_map():
        while 1:
            ...
            if x in corner and y in corner:
                if x == 0 and ((dungeon[1][y] == 0) and (dungeon[2][y] == 0)) and (
                        dungeon[x + 1][y] == 0 and dungeon[x + 2][y] == 0):
                    return x, y

                elif x == 4 and ((dungeon[4][y] == 0) and (dungeon[3][y] == 0)) and (
                        dungeon[x - 1][y] == 0 and dungeon[x - 2][y] == 0):
                    return x, y

                elif y == 0 and ((dungeon[x][1] == 0) and (dungeon[x][2] == 0)) and (
                        dungeon[x][y + 1] == 0 and dungeon[x][y + 2] == 0):
                    return x, y

                elif y == 4 and ((dungeon[x][4] == 0) and (dungeon[x][3] == 0)) and (
                        dungeon[x][y - 1] == 0 and dungeon[x][y - 2] == 0):
                    return x, y

            if dungeon[x][y] == 0 and (x in corner or y in corner):  # 중복 및 테두리 부분 나올 때 까지 랜덤 돌리기
                return x, y

    for i in range(3):
        x, y = landmark_pos[i][0: 2]

        if dungeon[x][y] != 0 or (x not in corner and y not in corner):  # 금지 구역을 생성 못할 경우 다시 돌리기
            x, y = random_map()
    ...
    for x, y, m in list(landmark_pos):
        dungeon[x][y] = m
        pos_x = 2
        pos_y = 2
        dir_x = 0
